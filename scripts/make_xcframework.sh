...
download_patched_version_of_bazel() {
    ...
    export JAVA_HOME="/usr/local/opt/openjdk@11/libexec/openjdk.jdk/Contents/Home" && "${BAZEL_DIR}"/4.1.0/bazel build --incompatible_restrict_string_escapes=false --incompatible_linkopts_to_linklibs=false -c opt //src:bazel
...
build_xcframework() {
    ...
    ${PATCHED_BAZEL_BIN} build --config=ios --cpu=ios_x86_64 -c opt tensorflow/lite/ios:${TENSORFLOW_TARGET}
    ...
    # Make ios slices
    ${PATCHED_BAZEL_BIN} build --config=ios --ios_multi_cpus=armv7,arm64 -c opt tensorflow/lite/ios:${TENSORFLOW_TARGET}
    unzip bazel-bin/tensorflow/lite/ios/${TENSORFLOW_TARGET}.zip -d "${BUILD_DIR}/iphones"

    # Create the xcframework
    xcrun xcodebuild -quiet -create-xcframework \
        -framework "${BUILD_DIR}/iphones/TensorFlowLiteC.framework" \
